...
const $themeButton = document.querySelector('.header__theme-switcher__button__container');
const colorThemes = [
    {
...
const setTheme = (theme) => {
    ...
    root.style.setProperty('--secondary-key-background', theme.secondaryKeyBackground);
    ...
    root.style.setProperty('--secondary-key-text-color', theme.secondaryKeyTextColor);
...
class Calculator {
    ...
    appendNumber(parameter) {
        if (parameter === '.' && this.currentValue.includes('.'))
            return;
        this.currentValue = this.currentValue + parameter;
    }
    chooseOperation(parameter) {
...
const numberFormatter = (parameter) => {
    let [integer, decimal] = parameter.split('.');
    integer = integer
        .split('')
        .reverse()
        .map((letter, index) => index === 0 ? letter : index % 3 === 0 ? letter + ',' : letter)
        .reverse()
        .join('');
